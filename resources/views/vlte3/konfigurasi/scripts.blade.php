<script>
    ...
    function clearProfileErrors() {
        const ids = ['kode_profile', 'nama_perusahaan', 'slogan', 'deskripsi', 'visi', 'misi', 'alamat', 'telepon',
            'email'
        ];
        ids.forEach(id => {
    ...
    async function submitCreateProfile(e) {
        ...
        try {
            ...
            const res = await fetch(`{{ url('admin/konfigurasi') }}`, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                },
    ...
    async function submitEditProfile(e) {
        ...
        try {
            ...
            const res = await fetch(`{{ url('admin/konfigurasi') }}/${id}`, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                },
            ...
            if (!res.ok) throw new Error('Gagal menyimpan');
            const data = await res.json();
            showNotification(res.message || 'Konfigurasi berhasil ditambahkan', 'success');
        } catch (err) {
            console.error(err);
            alert('Gagal menyimpan');
        } finally {
