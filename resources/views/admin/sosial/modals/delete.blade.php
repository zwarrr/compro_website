<div id="deleteSosmedModal" class="fixed inset-0 bg-gray-900 bg-opacity-60 hidden items-center justify-center z-50 transition-opacity duration-300 backdrop-blur-sm">
  <div class="modal-content bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 transform transition-all duration-300 scale-95 opacity-0">
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50 rounded-t-xl">
      ...
      <button onclick="closeDeleteSosmedModal()" class="text-gray-400 hover:text-gray-600 transition-colors rounded-full p-1 hover:bg-gray-200">
    ...
    <div class="p-8">
      ...
      <div class="text-center mb-6">
        ...
        <p class="text-gray-600 mb-4">Anda akan menghapus sosial media:</p>
        <div class="bg-gray-50 rounded-lg p-4 mb-4">
          <span class="font-bold text-lg text-gray-900" id="delete_sosmed_title"></span>
        </div>
      ...
      <form id="deleteSosmedForm" onsubmit="submitDeleteSosmed(event)">
        <input type="hidden" id="delete_sosmed_id" name="id">
        <div class="flex gap-3">
          <button type="button" onclick="closeDeleteSosmedModal()" class="flex-1 px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-semibold">Batal</button>
          <button type="submit" id="deleteSosmedSubmitBtn" class="flex-1 px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-semibold flex items-center justify-center gap-2">Ya, Hapus</button>
        </div>
      </form>
    </div>
  </div>
